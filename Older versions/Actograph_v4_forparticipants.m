...
dayDates = startTime + days(0:(numDays-1))';           % e.g. 2025-04-23, 2025-04-24, …
weekDays = cellstr(datestr(dayDates,'dddd'));          % 'Wednesday', 'Thursday', …
...
for wk = 1:nWeeks
    ...
    %% 4a) Participant Activity Profile (auto-scaled Y-axis)
    fig = figure('Color','w', ...
    ...
    %% 4d) Daily Light Exposure Bar Chart
...
%% 6) Call-Out Sheet for Quantitative Summary + Rhythm Metrics (IS, IV + Normative Ranges)
days       = (1:numDays)';
totalAct   = nansum(binnedActivity,2);
hoursLight = sum(binnedLight>1,2) / 60;

% Peak activity time (hh:mm:ss)
...
% Light start/end times (hh:mm:ss)
...
M  = binnedActivity;         % days × minutes
mh = nanmean(M,1);           % mean per minute across days
m  = nanmean(M(:));          % grand mean
...
% Normative ranges:
%   IS: 0.58–0.73, IV: 0.56–0.77  
%   — derived from actigraphy in >6 000 free-living U.S. adults  
%     (NHANES 2003–2006; Landry et al. Sleep 2015; van Someren et al. J Biol Rhythms 2017)
IS_norm = "0.58-0.73";
IV_norm = "0.56-0.77";

% Only fill the global metrics & ranges in the first row:
...
import mlreportgen.ppt.*;  % requires MATLAB Report Generator toolbox

pptFile = fullfile(outputFolder, 'AllFigures_Report.pptx');
ppt     = Presentation(pptFile);
open(ppt);

% Add each .jpg as a slide
